...
exports.AutoMarkingService = exports.calculateOverallBand = exports.calculateSectionScore = exports.convertToBandScore = void 0;
...
const convertToBandScore = (rawScore) => {
...
exports.convertToBandScore = convertToBandScore;
    ...
    if (Array.isArray(correctAnswer)) {
        if (Array.isArray(userAnswer)) {
            ...
            if (userAnswer.length !== correctAnswer.length)
                return false;
            return userAnswer.every((ans, idx) => normalizeAnswer(ans) === normalizeAnswer(correctAnswer[idx]));
        }
        // User gave single answer but correct is array - check if any match
        return correctAnswer.some((correct) => normalizeAnswer(userAnswer) === normalizeAnswer(correct));
    }
...
const calculateSectionScore = (userAnswers, sections) => {
    let correctCount = 0;
    // Flatten all questions from sections
    const allQuestions = sections.flatMap((section) => section.questions);
    for (const userAnswer of userAnswers) {
        const question = allQuestions.find((q) => q.questionNumber === userAnswer.questionNumber);
...
exports.calculateSectionScore = calculateSectionScore;
...
exports.AutoMarkingService = {
    calculateSectionScore: exports.calculateSectionScore,
    convertToBandScore: exports.convertToBandScore,
    calculateOverallBand: exports.calculateOverallBand,
};
